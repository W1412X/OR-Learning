...
"""本模块实现公交司机排班问题（基于流模型的 CP-SAT 求解）。
...
# 命令行参数（argparse）：
#   --instance          默认 1：算例编号（1=SMALL, 2=MEDIUM, 3=LARGE）
#   --output_proto_file 默认 ""：把 cp_model proto 写入的文件（为空则不导出）
#   --params            默认 ""：传给 CP-SAT 求解器的参数（文本格式）
...
def find_minimum_number_of_drivers(shifts, params):
    ...
    # 所有时长均以分钟为单位。
    max_driving_time = 540  # 9 小时（每司机累计驾驶时间上限；原注释误写为 8 小时，实际 540 分钟 = 9 小时，与模块 docstring 一致）。
    ...
    extra_time = 10 + 25  # 司机路径起点计入的固定附加时间（首班前准备 + 末班后收尾等）
    max_break = 180  # 班次间隔超过 180 分钟则不再衔接（换司机）
    ...
    # 我们将构建一张"从一天开始到一天结束"的流网络。
    #
    # 沿着网络中的一条路径（即一位司机一天的工作序列），逐段累计：
    # 驾驶时间、自上次休息以来的连续驾驶时间、总工作时间。

    model = cp_model.CpModel()

    # 每个节点（班次）的状态变量：
    #   driving_time[shift]          到达该班次时累计的驾驶时间
    #   working_time[shift]          到达该班次时累计的总工作时间
    #   no_break_driving_time[shift] 自上次休息以来累计的连续驾驶时间
    driving_time = {}
    working_time = {}
    ...
    for shift in range(num_shifts):
        ...
        # 从源点到班次的弧（表示司机的一天从该班次开始）：
        #    - 设置该司机的初始工作时间
        #    - 初始化驾驶时间与"自上次休息以来的连续驾驶时间"
        ...
        # 从班次到汇点的弧（表示司机在该班次后结束一天的工作）：
        #     - 校验总工作时间不低于 min_working_time（软性最短工时）
    ...
    # 约束（流守恒/DAG）：每个班次节点恰好有一条入弧和一条出弧。
    for shift in range(num_shifts):
        model.Add(sum(outgoing_literals[shift]) == 1)
        model.Add(sum(incoming_literals[shift]) == 1)

    # 决策变量：司机数量（下界为理论下限 min_num_drivers，上界取其 3 倍）；
    # 进入汇点的弧数 = 离开源点的弧数 = 司机数量。
...
def main(args):
    ...
    # 第二阶段（本示例未启用）：在司机数量固定的前提下最小化总工作时间。
