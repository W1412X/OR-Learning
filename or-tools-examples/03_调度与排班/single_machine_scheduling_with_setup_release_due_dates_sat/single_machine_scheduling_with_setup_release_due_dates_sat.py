...
def single_machine_scheduling():
    ...
    # 机器不重叠约束：同一时刻机器只能加工一个作业。
    model.add_no_overlap(intervals)

    # ----------------------------------------------------------------------------
    # 用电路（circuit）约束建模换型时间。
    arcs = []
    for i in all_jobs:
        # 从哑节点（0）到作业的起始弧。
        start_lit = model.new_bool_var("")
        arcs.append((0, i + 1, start_lit))
        # 若该作业是第一个作业，则强制其开始时间为最早可行时间。
        ...
        for j in all_jobs:
            ...
            # 添加条件化（reified）先后约束，把弧字面量与两个作业的时间关联。
            # 若 release_dates[j] == 0，由于在最小化 makespan，
            # 可以把该不等式强化为等式。
