...
- 每位司机的驾驶时间上限 <= 9 小时
- 每位司机的工作时间上限 <= 12 小时
- 每位司机的工作时间下限 >= 6.5 小时（软约束）
- 每驾驶 4 小时后必须休息 30 分钟
- 第一个班次前有 10 分钟准备时间
- 最后一个班次后有 15 分钟清扫时间
- 每个班次后有 2 分钟等待乘客上下车的时间
"""
...
# absl flags 命令行参数：
#   --output_proto  默认 ""：把 cp_model proto 写入的文件（为空则不导出）
#   --params        默认 "num_search_workers:16,log_search_progress:true,
#                   max_time_in_seconds:30"：传给 CP-SAT 求解器的参数（文本格式）
#   --instance      默认 0：选择算例（0=TINY, 1=SMALL, 2=MEDIUM, 3=LARGE）
...
SAMPLE_SHIFTS_TINY = [
    ...
    # 列说明：
    # - 班次编号
    # - 班次开始时间，hh:mm 字符串（仅用于日志输出与可读性）
    # - 班次结束时间，hh:mm 字符串（仅用于日志输出与可读性）
    # - 班次开始时间（分钟）
    # - 班次结束时间（分钟）
    # - 班次时长（分钟）
    #
...
def bus_driver_scheduling(minimize_drivers: bool, max_num_drivers: int) -> int:
    ...
    若 minimize_drivers 为 True，目标是最少司机数，不关心各司机的工作时长。

    否则，将创建 max_num_drivers 名"非可选"司机，并最小化这些司机的
    工作时间之和。

    Args:
      minimize_drivers: 布尔参数，指定问题目标。为 True 时尝试最小化使用的
        司机数；为 False 时最小化各位司机的工作时间之和。
      max_num_drivers: 指定"非可选"司机的确切数量；仅在 'minimize_drivers'
        为 False 时使用。
    ...
    # 所有时长均以分钟为单位。
    max_driving_time = 540  # 9 小时（每司机累计驾驶时间上限；原注释误写为 8 小时，540 分钟 = 9 小时，与模块 docstring 一致）。
    max_driving_time_without_pauses = 240  # 4 小时（不休息连续驾驶上限）
    min_pause_after_4h = 30  # 驾驶 4 小时后要求的最小休息时长
    min_delay_between_shifts = 2  # 相邻班次间的最小间隔（含上下客等待）
    max_working_time = 720  # 12 小时（每司机总工作时间上限）
    min_working_time = 390  # 6.5 小时（软性最小工作时间）
    setup_time = 10  # 首班前的准备时间
    cleanup_time = 15  # 末班后的清扫时间
    ...
    # 对每位司机和每个班次，我们存储：
    #   - 包含该班次在内的累计总驾驶时间
    #   - 自上次 30 分钟休息以来的累计驾驶时间
    # 特殊弧的作用：
    #   - 'from source to shift'（源点→班次）：设置司机的开始时间，并累计第一个班次
    #   - 'from shift to end'（班次→汇点）：设置司机的结束时间，并填入该司机的
    #     driving_times 变量
    # 两个班次之间弧的作用：
    #   - 把班次时长累加到总驾驶时间
    #   - 若两个班次间隔超过 30 分钟，则重置累计驾驶时间；否则把班次时长
    #     累加到"自上次休息以来的驾驶时间"

    # 每个（司机, 班次）组合的变量：总驾驶时间、是否执行该班次、
    # 自上次休息以来的驾驶时间
    total_driving = {}
    no_break_driving = {}
    performed = {}
    ...
    for d in range(num_drivers):
        ...
        for s in range(num_shifts):
            ...
            # 从源点到班次的弧（司机的一天从该班次开始）：
            #    - 设置司机的开始时间
            #    - 初始化累计驾驶时间与"自上次休息以来的驾驶时间"
            ...
            # 注意：~performed 是取反字面量，不加入共享列表；因为从全局看，
            # 每个节点恰好有一条入弧和一条出弧。
            ...
            for o in range(num_shifts):
                ...
                # 根据班次间隔决定：间隔 >= 30 分钟则重置连续驾驶时间，否则继续累计。
    ...
    for d in range(num_drivers):
        ...
        for s in range(num_shifts):
            ...
            # 技巧：检测上一班次与当前班次之间的等待时间是否超过 30 分钟——
            # 此时 no_break_driving 会被重置，据此判断是否打印休息标记。
...
def main(_):
    ...
    # 第一阶段：最小化司机数（minimize_drivers=True）。
