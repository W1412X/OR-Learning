...
参见：https://en.wikipedia.org/wiki/Golomb_ruler
"""
...
# flag：CP-SAT 求解器参数（键:值 逗号分隔），默认 16 线程、打印日志、限时 45 秒
...
def solve_golomb_ruler(order: int, params: str) -> None:
    ...
    # 目标函数：最小化尺长（最后一个刻度的位置）
    model.minimize(marks[order - 1])

    # 求解模型。
    solver = cp_model.CpSolver()
    if params:
        # 从 "键:值" 文本解析求解器参数（对应 --params flag）
